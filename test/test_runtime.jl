if !KRILL_FAST_TESTS
            ...
            if occursin("getUpdates", url)
                ...
                if call_count[] == 1
                    return HTTP.Response(200, JSON3.write(Dict(
                        "ok" => true,
                        "result" => Any[
                            Dict("update_id" => 500, "message" => Dict(
                                "message_id" => 1, "text" => "hello",
                                "chat" => Dict("id" => 42),
                                "from" => Dict("id" => 42),
                            )),
                            Dict("update_id" => 501, "message" => Dict(
            ...
            if occursin("getUpdates", url)
                ...
                if call_count[] <= 2
                    return HTTP.Response(200, JSON3.write(Dict(
                        "ok" => true,
                        "result" => Any[
                            Dict("update_id" => 600, "message" => Dict(
    ...
    @testset "shutdown is clean" begin
        ...
        runtime = RuntimeState(TelegramChannel(client; poll_timeout=0, poll_interval=0.01); workspace=mktempdir())
            ...
            if occursin("getUpdates", url)
                ...
                if call_count[] == 1
                    return HTTP.Response(200, JSON3.write(Dict(
                        "ok" => true,
                        "result" => Any[
                            Dict("update_id" => 700 + i, "message" => Dict(
