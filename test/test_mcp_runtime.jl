...
function _MCPMod._rpc(client::MockMCPClient, method::String, params=nothing; timeout_s::Real=5.0)
    ...
    elseif method == "tools/call"
        ...
        tool_args = params isa AbstractDict ? Dict{String,Any}(String(k) => v for (k, v) in get(params, "arguments", Dict{String,Any}())) : Dict{String,Any}()
...
@testset "Krill.jl MCP module" begin
    ...
    @testset "MCPServer constructor validation" begin
        ...
        # stdio with url
        @test_throws ArgumentError MCPMod.MCPServer(name="s", transport=:stdio, command="cmd", url="http://x")
        # HTTP without url
    ...
    @testset "_render_tool_result" begin
        ...
        r_multi = Dict{String,Any}("content" => Any[
        ...
        mock_telegram_request = function(method, url, headers, body)
            if occursin("getUpdates", url)
                return HTTP.Response(200, JSON3.write(Dict(
                    "ok" => true,
                    "result" => Any[
                        Dict("update_id" => 7100, "message" => Dict(
                            "message_id" => 1,
                            "text" => "hello",
                            "chat" => Dict("id" => 9),
                            "from" => Dict("id" => 9),
    ...
    @testset "RuntimeState rejects MCP config without llm_provider" begin
        client = TelegramClient("token"; base_url="https://example.test/botTOKEN", request=(args...)->HTTP.Response(200, JSON3.write(Dict("ok" => true, "result" => Any[]))))
