...
@testset "mixed outcomes" begin
    ...
    start_dispatch!(mgr)
    publish_outbound!(hub, make_msg(text="ok1", delivery_policy=DeliveryPolicy(max_retries=0, timeout_ms=0)))
    publish_outbound!(hub, make_msg(text="fail", delivery_policy=DeliveryPolicy(max_retries=0, timeout_ms=0)))
    publish_outbound!(hub, make_msg(text="ok2", delivery_policy=DeliveryPolicy(max_retries=0, timeout_ms=0)))
    sleep(0.5)
    stop_dispatch!(mgr)
