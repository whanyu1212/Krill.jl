    ...
    return function(msg::InboundMessage, history::Vector{TurnRecord})
        memory_text = if memory_store === nothing
            ...
            try
                ...
                @warn "failed to load session memory for context build" session_key=msg.session_key exception=(e, catch_backtrace())
                nothing
            end
        end
        ...
        if instructions_builder !== nothing
            try
                ...
                @warn "failed to build prompt instructions; falling back to system_prompt" session_key=msg.session_key exception=(e, catch_backtrace())
                system_prompt_for_turn = system_prompt
                memory_for_context = memory_text
            end
        end
