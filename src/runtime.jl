...
function RuntimeState(
    ...
    if agent.builtin_tools.enable
        ...
        send_tool_message =
            (chat_id, text; disable_web_page_preview = false) -> begin
    ...
    if !isempty(agent.mcp_servers)
        resolved_tool_registry === nothing && (resolved_tool_registry = ToolRegistry())
        mcp_connections =
            connect_mcp_servers!(resolved_tool_registry, agent.mcp_servers; connect_fn = agent.mcp_connect_fn)
    ...
    if consumer === nothing
        ...
        consumer =
            (hub, running) -> run_session_loop!(hub, running;
                store = store,
                processor = processor,
...
function status(rt::RuntimeState)
    last_send_at =
        rt.last_successful_send_at[] === nothing ?
    ...
    return Dict{String,Any}(
        ...
        "subagents_running" =>
            rt.subagent_manager === nothing ? 0 : subagent_count(rt.subagent_manager; running_only = true),
    )
end
